	...
	'flickity':function(){//手机滑动
		if($(".ff-gallery").length){
			...
			$("<link>").attr({
				rel: "stylesheet",
				type: "text/css",
				href: maccms.path +"template/jquery.flickity/2.1.1/flickity.min.css"
			}).appendTo("head");
			$.getScript(maccms.path +"template/jquery.flickity/2.1.1/flickity.pkgd.min.js", function(){
...
'image': {//图片
	'lazyload': function(){//延迟加载
		...
		$.getScript(maccms.path +"template/youku/js/jquery.lazyload.min.js", function(response, status) {
			$("img.ff-img").lazyload({
				placeholder : maccms.path +"template/youku/images/no.jpg",
				effect : "fadeIn",
				failurelimit: 15
				//threshold : 400
	...
	'qrcode': function(){//生成二维码
		...
		$(".glyphicon-phone,.fn-download").on('show.bs.popover', function () {
			$(".glyphicon-phone").attr('data-content','<img class="ff-qrcode" src="//cdn.feifeicms.co/qrcode/1.0/?w=120&h=120&url='+encodeURIComponent(feifei.browser.url)+'"/>');
		})
	},
	'vcode':function(){//安全码
		return '<label><img class="ff-vcode-img" src="'+maccms.path +'index.php?s=Vcode-Index"></label>';
	},
	'slide':function(){//轮播幻灯
...
'vcode': {//验证码
	...
	'click': function(){//点击刷新
		$('body').on('click', 'img.ff-vcode-img', function(){
			$(this).attr('src', maccms.path +'index.php?s=Vcode-Index');
		});
	}
},
'search': {//搜索
			...
			if(event.keyCode == 13){
				...
				if($form .attr('data-sid') == 2){
					if($form .attr('data-action-news')){
						...
						$form.attr('action', maccms.path +'index.php?s=news-search');
					}
				}else{
					if($form.attr('data-action')){
						...
						$form.attr('action', maccms.path +'index.php?s=vod-search');
					}
				}
				if($action){
		...
		$(".ff-search").on("submit", function(){
			...
			if($(this).attr('data-sid') == 2){
				if($(this).attr('data-action-news')){
					...
					$(this).attr('action', maccms.path +'index.php?s=news-search');
				}
			}else{
				if($(this).attr('data-action')){
					...
					$(this).attr('action', maccms.path +'index.php?s=vod-search');
				}
			}
			if($action){
				location.href = $action.replace('FFWD',encodeURIComponent($wd));
	...
	'autocomplete': function(){//3.5增加搜索结果控制
		...
		if( $limit > 0){
			...
			$.getScript("/template/youku/js/jquery.autocomplete.min.js", function(response, status) {
				if($('.ff-search').eq(0).attr('data-sid') == 2){
					$ajax_url = maccms.path +'index.php?g=home&m=search&a=news';
				}else{
					$ajax_url = maccms.path +'index.php?g=home&m=search&a=vod';
				}
				$('.ff-wd').autocomplete({
					serviceUrl : $ajax_url,
					params: {'limit': $limit},
	...
	'hot':function(){
		...
		$("#ff-site-hot").load(maccms.path +"index.php?s=ajax-site_hot");
	}
},
'record': {//云记录
	'load': function(){
	...
	'get':function(){
		...
		if($(".ff-record-get").eq(0).length){
			$.get(maccms.path +'index.php?g=home&m=record&a=vod&sid=1', function(data){
				if(data == ''){
	...
	'set':function(){//用户互动记录
		...
		if($(".ff-record-set[data-type=1]").eq(0).attr('data-sid')){
			...
			$.get(maccms.path +'index.php?g=home&m=record&a=post&sid='+$this.attr("data-sid")+'&did='+$this.attr("data-id")+'&type=1&did_sid='+$this.attr("data-id-sid")+'&did_pid='+$this.attr("data-id-pid"));
		}
		//喜欢 想看 在看 看过 写入记录
		$('body').on('click', 'a.ff-record-set', function(e){
			...
			if($(this).attr("data-id")){
				$.ajax({
					url: maccms.path +'index.php?g=home&m=record&a=post&sid='+$(this).attr("data-sid")+'&did='+$(this).attr("data-id")+'&type='+$(this).attr("data-type"),
					cache: false,
					dataType: 'json',
	...
	'del':function(){//删除记录
		$('body').on('click', 'a.ff-record-delete', function(e){
			...
			if($(this).attr("data-id")){
				$.ajax({
					url: maccms.path +'index.php?g=home&m=record&a=delete&id='+$(this).attr("data-id"),
					cache: false,
					dataType: 'json',
					success: function(json){
						if(json.status == 200){
				...
				$(".ff-score").each(function(i){
					$(".ff-score").eq(i).find('.ff-score-raty').raty({ 
						...
						click: function(score, evt) {
							...
							$.ajax({
								type: 'get',
								url: maccms.path +'index.php?s=gold-'+$(this).attr('data-module')+'-id-'+$(this).attr('data-id')+'-score-'+(score*2),
								timeout: 5000,
								dataType:'json',
								error: function(){
...
'updown':{//顶踩
	'click': function(){
		$('body').on('click', 'a.ff-updown-set', function(e){
			...
			if($(this).attr("data-id")){
				$.ajax({
					url: maccms.path +'index.php?s=updown-'+$(this).attr("data-module")+'-id-'+$(this).attr("data-id")+'-type-'+$(this).attr("data-type"),
					cache: false,
					dataType: 'json',
					success: function(json){
						$this.addClass('disabled');
...
'hits':{//人气
	'load': function(){
		$(".ff-hits").each(function(i){
			...
			$.ajax({
				url: maccms.path +'index.php?s=hits-show-id-'+$this.attr("data-id")+'-sid-'+$this.attr("data-sid")+'-type-'+$this.attr("data-type"),
				cache: true,
				dataType: 'json',
				success: function(json){
...
'forum': {//讨论模块功能
	...
	'system': function(){
		...
		$('body').on('click', 'a.forum-report', function(){
			...
			if($id){
				$.ajax({
					type: 'get',
					url: maccms.path +'index.php?s=forum-report-id-'+$id,
					timeout: 3000,
					dataType:'json',
					success:function(json){
		...
		$('body').on('click', 'a.forum-delete', function(){
			...
			if($id){
				$.ajax({
					type: 'get',
					url: maccms.path +'index.php?s=forum-delete-id-'+$id,
					timeout: 3000,
					dataType:'json',
					success:function(json){
						if(json.status == 200){
	...
	'ajax': function($target, $sid, $cid, $pid, $ismore, $page){//AJAX加载系统评论
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=forum-ajax-sid-'+$sid+'-cid-'+$cid+'-pid-'+$pid+'-ismore-'+$ismore+'-p-'+$page,
			timeout: 3000,
			error: function(){
	...
	'reply': function($id){//更新回复数及显示回复链接
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=forum-reply-id-'+$id,
			timeout: 3000,
			dataType:'json',
			success:function(json){
				if(json.status==200){
...
'playurl': {//播放地址
	'download': function(){
		if($(".ff-playurl-down").length){
			$.getScript("/template/youku/js/xunlei.js",function(){
				//3.4增加鼠标点击去掉文件名标题
	...
	'tongji': function(){
		if($("#cms_player").length){
			$.getScript("//cdn.feifeicms.co/tongji/3.8/");
		}
	},
	'active':function(){
		...
		if($status != 200){
			if($trysee > 0){
				window.setTimeout(function(){
					$.get(maccms.path +'index.php?s=vod-vip-action-trysee-id-'+$vod_id+'-sid-'+$vod_sid+'-pid-'+$vod_pid, function(html){
			...
			$('#cms_player').on("click",".vod-price",function(){
				$(this).html('Loading...');
				$.get(maccms.path +'index.php?s=vod-vip-action-ispay-id-'+$vod_id+'-sid-'+$vod_sid+'-pid-'+$vod_pid, function(json){
					if(json.status == 200){
						location.reload();
...
'user':{
	...
	'islogin':function(){
		if($('.ff-user').length && (cms.urlhtml == 1)){
			$.ajax({
				type: 'get',
				url: maccms.path +'index.php?s=user-info',
				timeout: 3000,
				dataType:'json',
				success:function(json){
					if(json.status==200){
	...
	'login':function(){
		...
		$.ajax({
			type: 'get',
			cache: false,
			url: maccms.path +'index.php?s=user-ajax_login',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
	...
	'ScoreUpvip':function(){
		...
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=user-ajax_upvip',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
	...
	'ChangeEmail':function(){
		...
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=user-ajax_email',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
	...
	'ChangePwd':function(){
		...
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=user-ajax_repwd',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
...
'order':{//影币订单模块
	'payment':function(){//在线充值付款界面
		...
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=payment-index',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
	...
	'card':function(){//卡密充值付款界面
		...
		$.ajax({
			type: 'get',
			url: maccms.path +'index.php?s=payment-card',
			timeout: 3000,
			success:function($html){
				$('body').append($html);
				$('.ff-modal').modal('show');
